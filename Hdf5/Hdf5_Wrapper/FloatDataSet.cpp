...
CFloatDataSet::CFloatDataSet(const std::string& strDataSetKeyName, InnerType::LP_DATA_HEADER pHeader, void* pData, int nCompressLevel /*= -1*/):
...
CFloatDataSet::~CFloatDataSet()
{
}

unsigned int CFloatDataSet::GetChildNum(void)
{
	return 0;
}

int CFloatDataSet::CreateSelfToFile(H5::Group* pGroupNode)
{
	using namespace std;
	using namespace H5;
...
int CFloatDataSet::ParseSelf(H5::DataSet* pDataset, InnerType::HDF5_READ_DATA& DstData)
{
	if (nullptr == pDataset)
	{
		return ERROR_PARAM_INVALID;
	...
	auto pScalarHandler = [](H5::DataSet* pDataset, InnerType::HDF5_READ_DATA& DatasetData)->int
	{
		...
		//数据类型
		auto DatasetDataType = pDataset->getFloatType();
		//申请内存
		char* pData = new char[nDatasetValueLen];
		if (nullptr != pData)
		{
			//数值类型
			DatasetData.Header.eumDataType = InnerType::DATA_TYPE::FLOAT_TYPE;
			//数值类型占用的字节数
...
int CFloatDataSet::ReadPartialData(H5::DataSet* pDataset, const InnerType::HDF5_QUERY_PARTIAL_DATA_PARAM& QueryParam, InnerType::HDF5_READ_DATA& DstData)
{
	using namespace H5;
	...
	//数值类型
	DstData.Header.eumDataType = InnerType::DATA_TYPE::FLOAT_TYPE;
	//数值类型占用的字节数
