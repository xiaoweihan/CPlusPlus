...
CIntDataSet::CIntDataSet(const std::string& strDataSetKeyName, InnerType::LP_DATA_HEADER pHeader, void* pData, int nCompressLevel /*= -1*/):
...
Author:xiaowei.han
*********************************************************/
unsigned int CIntDataSet::GetChildNum(void)
{
	return 0;
}
...
Author:xiaowei.han
*********************************************************/
int CIntDataSet::CreateSelfToFile(H5::Group* pGroupNode)
{
	using namespace std;
	using namespace H5;
	...
	if (m_pHeaderInfo->nRank > 0)
	{
		...
		if (pDims)
		{
			...
			if (m_nCompressLevel >= 0)
			{
				...
				pDataSet->write(m_pData, *pSuitableDataType);
...
Author:xiaowei.han
*********************************************************/
int CIntDataSet::ParseSelf(H5::DataSet* pDataset, InnerType::HDF5_READ_DATA& DstData)
{
	if (nullptr == pDataset)
	{
		return ERROR_PARAM_INVALID;
	...
	auto pScalarHandler = [](H5::DataSet* pDataset, InnerType::HDF5_READ_DATA& DatasetData)->int
	{
		...
		{
			//数值类型
			DatasetData.Header.eumDataType = InnerType::DATA_TYPE::INT_TYPE;
			//数值类型占用的字节数
			...
			//无符号
			if (H5T_SGN_NONE == DatasetDataType.getSign())
			{
			...
			DatasetData.Header.AdditionalDataType = IntHeader;
			//创建所需的数据类型
...
/*********************************************************
FunctionName:ReadPartialData
FunctionDesc:解析自身部分数据
InputParam:
OutputParam:
Return:
Author:xiaowei.han
*********************************************************/
int CIntDataSet::ReadPartialData(H5::DataSet* pDataset, const InnerType::HDF5_QUERY_PARTIAL_DATA_PARAM& QueryParam, InnerType::HDF5_READ_DATA& DstData)
{
	using namespace H5;
	...
	//数值类型
	DstData.Header.eumDataType = InnerType::DATA_TYPE::INT_TYPE;
	//数值类型占用的字节数
	...
	DstData.Header.AdditionalDataType = IntHeader;
	...
	//进行读取
	pDataset->read(pBuffer, *pSuitableDataType, memspace, DatasetDataSpace);
