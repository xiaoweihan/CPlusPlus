...
CFloatAttr::CFloatAttr(const std::string& strAttrKeyName, int nRank, int* pDimensionArray, unsigned int nDataByte, void* pData):
...
CFloatAttr::~CFloatAttr()
{
}

void CFloatAttr::WriteSelf(H5::DataSet* pElementDataSet)
{
	...
	using namespace H5;
	//创建所需的数据类型
	shared_ptr<CAbstractDataType> pDataTypeGenerator(DataTypeFactory::CreateDataType(InnerType::DATA_TYPE::FLOAT_TYPE, m_nDataByte,false));
...
void CFloatAttr::WriteSelf(H5::Group* pGroupNode)
{
...
Author:xiaowei.han
*********************************************************/
void CFloatAttr::ReadSelf(H5::Attribute* pAttr, InnerType::DATA_ATTRIBUTE& DataAttr)
{
	if (nullptr == pAttr)
	{
		return;
	...
	auto pScalarHandler = [](H5::Attribute& Attr, InnerType::DATA_ATTRIBUTE& DataAttr)
	{
		...
		//数据类型
		auto AttrDataType = Attr.getFloatType();
		//申请内存
		...
		if (nullptr != pData)
		{
			...
			//数值类型
			DataAttr.Header.eumDataType = InnerType::DATA_TYPE::FLOAT_TYPE;
			//数值类型占用的字节数
